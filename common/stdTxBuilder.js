    ...
    return __generator(this, function(_e) {
      switch (_e.label) {
        case 0:
          ...
          return [
            4 /*yield*/,
            walletProvider.getTxBuilderConfig(context, config)
          ];
        case 1:
          config = _e.sent();
          tempFee = config.fee;
          if (Array.isArray(tempFee)) {
            for (i = 0; i < tempFee.length; i++) {
              if (!(tempFee[i] instanceof coin_1.Coin)) {
                ...
                configFee[i] = new coin_1.Coin(
                  fee_1.denom,
                  big_integer_1.default(fee_1.amount.toString())
                );
              }
            }
          } else {
            if (!(tempFee instanceof coin_1.Coin)) {
              config.fee = new coin_1.Coin(
                tempFee.denom,
                big_integer_1.default(tempFee.amount.toString())
              );
            }
          }
          ...
          if (config.accountNumber) {
            config.accountNumber = big_integer_1.default(
              config.accountNumber.toString()
            );
          }
          _e.label = 2;
        case 2:
        ...
        case 4:
          ...
          if (
            !(
              accountNumber.lt(big_integer_1.default(0)) ||
              sequence.lt(big_integer_1.default(0))
            )
          )
            return [3 /*break*/, 6];
          return [
        ...
        case 5:
          ...
          _e.label = 6;
        case 6:
          signDoc = new stdTx_1.StdSignDoc(
            context.get("codec"),
            accountNumber,
            context.get("chainId"),
            stdFee,
